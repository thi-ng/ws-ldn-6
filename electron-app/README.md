...
You can edit the `src/cljs/electon_app/core.cljs` file and the changes should show up in the electron app without the need to re-launch.
...
user> (def figwheel-config
        {:figwheel-options {:css-dirs ["app/css"]}
         ...
         :all-builds
           [{:id "dev"
             :figwheel {:on-jsload "electon-app.core/mount-root"}
             :source-paths ["src/cljs" "env/dev/cljs"]
             :compiler {:main "electon-app.dev"
                        :asset-path "js/p/out"
